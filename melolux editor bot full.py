...
async def button_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "back_main":
        ...
        await render(query, tr("which_tag", lang), reply_markup=tags_menu_keyboard(lang))

    elif data.startswith("tag_"):
        field = data.replace("tag_", "")
        if field == "remove_all":
            await remove_all_tags(context)
            await render(query, tr("all_tags_removed", lang), reply_markup=main_menu_keyboard(lang))
        elif field == "picture":
        ...
        await render(query, tr("choose_bitrate", lang), reply_markup=bitrate_keyboard(lang))

    elif data.startswith("bitrate_"):
        bitrate = data.replace("bitrate_", "")
        await render(query, tr("applying_bitrate", lang))
        await run_ffmpeg_bitrate(context, bitrate)
        await render(query, tr("bitrate_done", lang, bitrate=bitrate), reply_markup=main_menu_keyboard(lang))

    elif data == "menu_volume":
        await render(query, tr("choose_volume", lang), reply_markup=volume_keyboard(lang))

    elif data.startswith("volume_"):
        percent = int(data.replace("volume_", ""))
        await render(query, tr("applying_volume", lang))
        await run_ffmpeg_volume(context, percent)
        await render(query, tr("volume_done", lang, percent=percent), reply_markup=main_menu_keyboard(lang))
        ...
        await render(query, tr("making_demo", lang))
