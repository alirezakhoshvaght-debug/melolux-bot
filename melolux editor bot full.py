...
def field_detail_keyboard(lang: str, field: str, current_value: str = None):
    ...
    if has_value:
        try:
            row2.append(InlineKeyboardButton(tr("field_copy", lang), copy_text=CopyTextButton(current_value)))
        except Exception:
...
async def button_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "back_main":
        ...
        if field == "remove_all":
            ...
            await render(query, text, reply_markup=field_detail_keyboard(lang, field, current_value))
...
def build_metadata_caption(context, path: Path, lang: str) -> str:
    ...
    filename = (
        context.user_data.get("custom_filename")
        or context.user_data.get("display_filename")
        or "Melolux Track"
    )
    title = g("TIT2")
    artist = g("TPE1")
