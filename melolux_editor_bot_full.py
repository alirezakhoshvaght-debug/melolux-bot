...
async def handle_audio(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    caption_text = build_metadata_caption(context, work_path, lang)
...
async def update_menu_message(context, status_text, keyboard, lang="fa"):
    """کپشن و منوی زیر فایل صوتی اصلی رو ویرایش می‌کنه (همراه با اطلاعات کامل موزیک) بدون فرستادن پیام جدید."""
    ...
    full_caption = status_text
    if work_path and Path(work_path).exists():
        full_caption = f"{status_text}\n\n{build_metadata_caption(context, Path(work_path), lang)}"
    try:
        await context.bot.edit_message_caption(
            chat_id=chat_id, message_id=message_id, caption=full_caption, reply_markup=keyboard
        )
    except Exception as e:
        logger.warning(f"خطا در آپدیت پیام منو: {e}")


async def refresh_audio_media(context, status_text, keyboard, lang="fa"):
    ...
    full_caption = f"{status_text}\n\n{build_metadata_caption(context, path, lang)}"
    ...
    try:
        ...
        await update_menu_message(context, status_text, keyboard, lang)
    finally:
        audio_file.close()
        if thumb_file:
            thumb_file.close()
...
async def button_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "back_main":
        ...
        meta = build_metadata_caption(context, Path(context.user_data["work_path"]), lang)
        await render(query, f'{tr("done_8d", lang)}\n\n{meta}', reply_markup=main_menu_keyboard(lang))
        ...
        meta = build_metadata_caption(context, Path(context.user_data["work_path"]), lang)
        await render(query, f'{tr("voice_sent", lang)}\n\n{meta}', reply_markup=main_menu_keyboard(lang))

    elif data == "action_apply_default":
        settings = load_user_settings(user_id)
        await apply_default_tags(context, settings)
        meta = build_metadata_caption(context, Path(context.user_data["work_path"]), lang)
        await render(query, f'{tr("default_applied", lang)}\n\n{meta}', reply_markup=main_menu_keyboard(lang))
...
async def _finalize_demo(update, context, seconds: int, lang: str):
    ...
    context.user_data["awaiting"] = None
    await update_menu_message(context, tr("main_menu_title", lang), main_menu_keyboard(lang), lang)
    try:
...
async def handle_text_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if awaiting == "reply_bitrate":
        ...
        context.user_data["awaiting"] = None
        await update_menu_message(context, tr("bitrate_done", lang, bitrate=bitrate), main_menu_keyboard(lang), lang)
        try:
            await status_msg.delete()
        except Exception:
            pass
    ...
    if awaiting == "reply_volume":
        ...
        context.user_data["awaiting"] = None
        await update_menu_message(context, tr("volume_done", lang, percent=percent), main_menu_keyboard(lang), lang)
        try:
            await status_msg.delete()
        except Exception:
            pass
    ...
    if awaiting == "cut":
        ...
        context.user_data["awaiting"] = None
        await update_menu_message(context, tr("cut_done", lang), main_menu_keyboard(lang), lang)
        try:
            await status_msg.delete()
        except Exception:
            pass
    ...
    if awaiting.startswith("tag_"):
        ...
        edited = context.user_data.setdefault("edited_tags", set())
        edited.add(field)
        await update_menu_message(context, tr("value_saved", lang), tags_menu_keyboard(lang, edited), lang)
        try:
            await update.message.delete()
        except Exception:
            pass
    ...
    if awaiting == "caption":
        ...
        context.user_data["awaiting"] = None
        await update_menu_message(context, tr("caption_saved", lang), main_menu_keyboard(lang), lang)
        try:
            await update.message.delete()
        except Exception:
            pass
...
async def handle_photo_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    edited = context.user_data.setdefault("edited_tags", set())
    edited.add("picture")
    await refresh_audio_media(context, tr("new_cover_applied", lang), tags_menu_keyboard(lang, edited), lang)
...
def build_metadata_caption(context, path: Path, lang: str) -> str:
    ...
    lines = [
        ...
        f"💿 Track N : {track}",
        f"🎙 Genre : {genre}",
        f"✍️ Lyrics : {lyrics[:60] + '…' if len(lyrics) > 60 else lyrics}",
        f"📝 Comment : {comment}",
